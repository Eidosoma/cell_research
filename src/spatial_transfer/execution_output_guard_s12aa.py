"""S12AA binding for the byte-frozen S12T actual-use dependency control.

The S12T registry remains byte-for-byte unchanged and governs dependency
imports, module origins, structural-input opens, caches, and signals.  S12AA
...
class OutputAccessViolation(DependencyPreflightError):
    """An S12AA output operation escaped its exact authorized roots."""
...
class OutputAccessRegistry:
    """Deny-first registry that can authorize only S12AA-owned output paths."""

    def __init__(self, raw: Mapping[str, Any], *, source: Path) -> None:
        self.raw = deepcopy(dict(raw))
        self.source = source.resolve()
        if (
            self.raw.get("schemaVersion") != "e07.s12aa.output-access-registry.v1"
            or self.raw.get("researchStepId") != "S12AA"
        ):
            raise DependencyPreflightError("unexpected S12AA output registry")
    ...
    def path_disposition(
        ...
        if any(_is_within(resolved, root) for root in self.prohibited_roots):
            ...
            allowed = True
            reason = "authorized_s12aa_output_root"
        else:
            allowed = False
            reason = "not_an_s12aa_output_path"
        return {
            "allowed": allowed,
            "reason": reason,
...
class OutputAccessLedger:
        ...
        *,
        ...
        with self._lock:
            event = {
                ...
                "eventKind": "s12aa_output_access",
                "operation": operation,
                "artifactClass": artifact_class,
                **disposition,
            }
            event["eventSha256"] = domain_hash(
                "E07/S12AA/output-event/v1",
                event,
            )
            self.events.append(event)
        if not disposition["allowed"]:
            raise OutputAccessViolation(
                f"denied S12AA output access: {disposition['reason']}: "
    ...
    def finalize(self) -> dict[str, Any]:
        ...
        ledger_sha = domain_hash("E07/S12AA/output-ledger/v1", events)
        authenticator = domain_hash(
            "E07/S12AA/output-ledger-authenticator/v1",
            {
        ...
        return {
            "schemaVersion": "e07.s12aa.output-access-ledger.v1",
...
def validate_output_ledger(
    ...
    *,
    ...
    reasons: list[str] = []
    if ledger.get("schemaVersion") != "e07.s12aa.output-access-ledger.v1":
    ...
    for ordinal, event in enumerate(events):
        ...
        if claimed != domain_hash("E07/S12AA/output-event/v1", body):
