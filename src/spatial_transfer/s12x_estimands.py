"""S12X integration of the outcome-independent S12W estimator contract.
...
def _slot_record(slot: FixedSlot) -> dict[str, Any]:
    ...
    record["slotCommitmentSha256"] = canonical_sha256(
        "E07/S12X/fixed-estimator-slot/v1",
        record,
    )
    return record
...
def build_fixed_slot_registry(
    ...
    registry = {
        "schemaVersion": "e07.s12x.fixed-estimator-slot-registry.v1",
        "researchStepId": "S12X",
    ...
    registry["semanticSha256"] = canonical_sha256(
        "E07/S12X/fixed-estimator-slot-registry/v1",
        registry,
    )
    return registry
...
def build_paired_estimands(
    ...
    payload = {
        ...
        "nonEvidentiaryRecordCount": sum(
            not row["evidentiary"] for row in adjusted
        ),
