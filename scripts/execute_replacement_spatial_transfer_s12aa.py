"""Execute the exact byte-frozen S12R replacement study as fresh step S12AA."""
...
from src.spatial_transfer.s12aa_estimands import (
    build_paired_estimands as build_s12w_paired_estimands,
)

REPOSITORY = Path(__file__).resolve().parents[1]
WORKSPACE = REPOSITORY.parent
ARTIFACTS = Path(os.environ.get("ARTIFACTS_DIR", "/artifacts"))
OUT = ARTIFACTS / "research_steps/S12AA"
S12R = ARTIFACTS / "research_steps/S12R"
CACHE = Path("/cache/e07-s12aa")
CONTROL = REPOSITORY / "configs/transfer/s12aa_fresh_s12r_execution.yaml"
OUTPUT_REGISTRY_FILE = REPOSITORY / "configs/transfer/s12aa_output_access_registry.json"
...
S12W = ARTIFACTS / "research_steps/S12W"
S12W_IMPLEMENTATION = REPOSITORY / "src/spatial_transfer/estimator_feasibility.py"
S12AA_ESTIMATOR = REPOSITORY / "src/spatial_transfer/s12aa_estimands.py"
S12Z = ARTIFACTS / "research_steps/S12Z"
S12Z_PROTOCOL_FILE = S12Z / "s12z_unified_elapsed_clock_protocol.yaml"
S12Z_PREREGISTRATION_FILE = S12Z / "preregistration_freeze.json"
S12Z_SOURCE_FREEZE_FILE = S12Z / "source_and_input_hash_freeze.json"
S12Z_COMMITMENTS_FILE = S12Z / "preserved_s12r_scientific_commitments.json"
S12Z_CLOCK_STATE_FILE = S12Z / "expected_clock_state_registry.json"
S12Z_FREEZE_REVALIDATION_FILE = S12Z / "freeze_revalidation.json"
S12Z_CONTRACT_PRESERVATION_FILE = S12Z / "scientific_contract_preservation.json"
S12Z_FIXED_FAMILY_FILE = S12Z / "fixed_family_conservation.json"
S12Z_PUBLISHER_FILE = S12Z / "fail_atomic_publication_qualification.json"
S12Z_VALIDATION_FILE = S12Z / "validation_summary.json"
S12Z_MANIFEST_FILE = S12Z / "artifact_manifest.json"
ELAPSED_CLOCK_IMPLEMENTATION = REPOSITORY / "src/morph2d/elapsed_clock.py"
...
def _finalize_output_access_ledger(*, disposition: str) -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError("S12AA output access ledger is not installed")
    ...
    if not validation["passed"]:
        raise RuntimeError("S12AA authenticated output ledger failed validation")
    return validation
...
def control_record() -> dict[str, Any]:
    ...
    if (
        ...
        or raw["historicalRelationship"]["retriesOrReusesS12U"] is not False
        or raw["historicalRelationship"]["retriesOrReusesS12X"] is not False
        ...
        or raw["frozenS12W"]["exactQualifiedImplementationRequired"] is not True
        or raw["frozenS12W"]["fixedFamilyConservationRequired"] is not True
        or raw["frozenS12W"]["S12UCachePermanentlyProhibited"] is not True
        or raw["frozenS12Z"]["exactQualifiedImplementationRequired"] is not True
        or raw["frozenS12Z"]["authenticatedElapsedClockRequiredOnNativeAndDsl"]
        is not True
        or raw["frozenS12Z"]["rawObservationLabelsScientificTime"] is not False
        or raw["frozenS12Z"]["fixedSlotCount"] != 7_986
        or raw["frozenS12Z"]["fixedHolmFamilyCount"] != 8
    ):
        raise RuntimeError("S12AA execution control is not the authorized continuation")
    body = {
        "schemaVersion": "e07.s12aa.execution-control-record.v1",
        ...
        "frozenS12W": raw["frozenS12W"],
        "frozenS12Z": raw["frozenS12Z"],
        "frozenPredecessorManifestHashes": raw["frozenPredecessorManifestHashes"],
    ...
    body["executionControlSha256"] = canonical_sha256(
        "E07/S12AA/execution-control/v1", body
    )
    return body
...
def _predecessor_immutability_validation() -> dict[str, Any]:
    # Validate only compact artifact manifests.  Never perform any filesystem
    # metadata or content operation on either consumed execution cache and
    # never open an S12U/S12X outcome-bearing row.
    raw_control = yaml.safe_load(CONTROL.read_text(encoding="utf-8"))
    expected_manifest_hashes = {
        str(step_id): str(expected)
        for step_id, expected in raw_control["frozenPredecessorManifestHashes"].items()
    }
    validations = {}
    for step_id, expected in sorted(expected_manifest_hashes.items()):
        path = ARTIFACTS / f"research_steps/{step_id}/artifact_manifest.json"
        actual = file_record(path)
        validations[step_id] = {
            "path": str(path),
            ...
            "actualSha256": actual["sha256"],
            "bytes": actual["bytes"],
            "passed": actual["sha256"] == expected,
            "manifestOnlyValidation": True,
        }
    s12t_baseline = read_json(S12T / "predecessor_immutability_baseline.json")
    result = {
        "schemaVersion": "e07.s12aa.predecessor-immutability-validation.v1",
        "researchStepId": "S12AA",
        ...
        "S12UCacheMetadataOperations": 0,
        "s12uCacheInspected": False,
        "s12uOutcomeArtifactsOpened": 0,
        "S12XCacheMetadataOperations": 0,
        "s12xCacheInspected": False,
        "s12xOutcomeArtifactsOpened": 0,
        "priorArtifactMutations": 0,
    }
    result["allPassed"] = bool(
        all(row["passed"] for row in validations.values())
...
def _s12w_estimator_validation() -> dict[str, Any]:
    raw = yaml.safe_load(CONTROL.read_text(encoding="utf-8"))["frozenS12W"]
    frozen_s12z = yaml.safe_load(CONTROL.read_text(encoding="utf-8"))["frozenS12Z"]
    files = {
        "protocol": S12W / "s12w_estimator_feasibility_repair_protocol.yaml",
        "preregistrationFreeze": S12W / "preregistration_freeze.json",
        "expectedStateDispositionRegistry": (
            S12W / "expected_state_disposition_registry.json"
        ),
        "preservedEstimandAndMultiplicity": (
            S12W / "preserved_estimand_and_multiplicity_commitment.json"
        ),
        "executionReviewGate": S12W / "s12w_execution_review_gate.json",
        "artifactManifest": S12W / "artifact_manifest.json",
        "qualifiedImplementation": S12W_IMPLEMENTATION,
    }
    expected = {
        "protocol": raw["protocolSha256"],
        "preregistrationFreeze": raw["preregistrationFreezeSha256"],
        "expectedStateDispositionRegistry": (
            raw["expectedStateDispositionRegistrySha256"]
        ),
        "preservedEstimandAndMultiplicity": (
            raw["preservedEstimandAndMultiplicitySha256"]
        ),
        "executionReviewGate": raw["executionReviewGateSha256"],
        "artifactManifest": raw["artifactManifestSha256"],
        # S12Z prospectively extended this exact S12W implementation only with
        # authenticated clock projection.  Its outcome-independent freeze is
        # revalidated separately before this current hash is accepted.
        "qualifiedImplementation": frozen_s12z["estimatorImplementationSha256"],
    }
    records = {
        key: {
            **file_record(path),
            "expectedSha256": expected[key],
            "passed": sha256_file(path) == expected[key],
        }
        for key, path in files.items()
    }
    gate = read_json(files["executionReviewGate"])
    commitment = read_json(files["preservedEstimandAndMultiplicity"])
    state = read_json(files["expectedStateDispositionRegistry"])
    checks = {
        "allHashesMatch": all(row["passed"] for row in records.values()),
        "qualificationPassed": gate["qualificationPassed"] is True,
        "s12rEstimandPreserved": gate["S12REstimandPreserved"] is True,
        "s12uCachePermanentlyProhibited": (
            gate["S12UCachePermanentlyProhibited"] is True
        ),
        "freshNamespaceRequired": gate["freshNamespaceRequiredForAnyFutureExecution"]
        is True,
        "separateExecutionDecisionWasRequired": (
            gate["scientificExecutionAuthorizedByS12W"] is False
            and gate["status"] == "qualified_pending_separate_human_execution_decision"
        ),
        "allReservedPopulation": (
            commitment["analysisPopulation"]
            == "all_reserved_rows_including_failures_and_censors"
        ),
        "fixedFamiliesExact": commitment["fixedFamilies"] == raw["fixedFamilies"],
        "holmExact": (
            commitment["multiplicityMethod"] == "Holm" and commitment["alpha"] == 0.05
        ),
        "nonEvidentiaryRuleExact": (
            commitment["infeasibleSlotDisposition"]["rawPValue"] == 1.0
            and commitment["infeasibleSlotDisposition"]["slotRetained"] is True
        ),
        "restrictedTimeExact": (
            commitment["restrictedTransitionTime"]["rightCensorTransition"] == 32
        ),
        "stateMatrixTotal": sum(state["expectedDispositionCounts"].values()) == 3_808,
        "s12wOriginalQualifiedHashRetained": raw["implementationSha256"]
        == "7a9d47ccc1d5bea99a8774f656c04da54cce44f47f9192515aaa5b64e2415a17",
    }
    return {
        "schemaVersion": "e07.s12aa.s12w-estimator-revalidation.v1",
        "researchStepId": "S12AA",
        "files": records,
        "checks": checks,
...
def _s12z_clock_validation() -> dict[str, Any]:
    raw = yaml.safe_load(CONTROL.read_text(encoding="utf-8"))["frozenS12Z"]
    files = {
        "protocol": S12Z_PROTOCOL_FILE,
        "preregistrationFreeze": S12Z_PREREGISTRATION_FILE,
        "sourceAndInputHashFreeze": S12Z_SOURCE_FREEZE_FILE,
        "preservedS12RCommitments": S12Z_COMMITMENTS_FILE,
        "expectedClockStateRegistry": S12Z_CLOCK_STATE_FILE,
        "freezeRevalidation": S12Z_FREEZE_REVALIDATION_FILE,
        "scientificContractPreservation": S12Z_CONTRACT_PRESERVATION_FILE,
        "fixedFamilyConservation": S12Z_FIXED_FAMILY_FILE,
        "failAtomicPublicationQualification": S12Z_PUBLISHER_FILE,
        "validationSummary": S12Z_VALIDATION_FILE,
        "artifactManifest": S12Z_MANIFEST_FILE,
        "elapsedClockImplementation": ELAPSED_CLOCK_IMPLEMENTATION,
        "baselineImplementation": REPOSITORY / "src/morph2d/baseline.py",
        "engineImplementation": REPOSITORY / "src/morph2d/engine.py",
        "dslAdapterImplementation": (
            REPOSITORY / "src/environment_suite/dsl_adapters.py"
        ),
        "spatialExecutionImplementation": (
            REPOSITORY / "src/spatial_transfer/execution.py"
        ),
        "estimatorImplementation": S12W_IMPLEMENTATION,
    }
    expected = {
        "protocol": raw["protocolSha256"],
        "preregistrationFreeze": raw["preregistrationFreezeSha256"],
        "sourceAndInputHashFreeze": raw["sourceAndInputHashFreezeSha256"],
        "preservedS12RCommitments": raw["preservedS12RCommitmentsSha256"],
        "expectedClockStateRegistry": raw["expectedClockStateRegistrySha256"],
        "freezeRevalidation": raw["freezeRevalidationSha256"],
        "scientificContractPreservation": raw["scientificContractPreservationSha256"],
        "fixedFamilyConservation": raw["fixedFamilyConservationSha256"],
        "failAtomicPublicationQualification": raw[
            "failAtomicPublicationQualificationSha256"
        ],
        "validationSummary": raw["validationSummarySha256"],
        "artifactManifest": raw["artifactManifestSha256"],
        "elapsedClockImplementation": raw["elapsedClockImplementationSha256"],
        "baselineImplementation": raw["baselineImplementationSha256"],
        "engineImplementation": raw["engineImplementationSha256"],
        "dslAdapterImplementation": raw["dslAdapterImplementationSha256"],
        "spatialExecutionImplementation": raw["spatialExecutionImplementationSha256"],
        "estimatorImplementation": raw["estimatorImplementationSha256"],
    }
    records = {
        key: {
            **file_record(path),
    ...
    protocol = yaml.safe_load(S12Z_PROTOCOL_FILE.read_text(encoding="utf-8"))
    preregistration = read_json(S12Z_PREREGISTRATION_FILE)
    freeze = read_json(S12Z_FREEZE_REVALIDATION_FILE)
    commitments = read_json(S12Z_COMMITMENTS_FILE)
    clock_states = read_json(S12Z_CLOCK_STATE_FILE)
    preservation = read_json(S12Z_CONTRACT_PRESERVATION_FILE)
    families = read_json(S12Z_FIXED_FAMILY_FILE)
    publisher = read_json(S12Z_PUBLISHER_FILE)
    validation = read_json(S12Z_VALIDATION_FILE)
    checks = {
        "allHashesMatch": all(row["passed"] for row in records.values()),
        "qualificationPassed": validation["allPassed"] is True
        and validation["gateCount"] == 12,
        "freshExecutionRequiredSeparateHumanDecision": validation[
            "freshExecutionAuthorized"
        ]
        is False,
        "protocolClockDomainExact": protocol["clockContract"]["horizonTransitions"]
        == 32
        and protocol["clockContract"]["initial"]["elapsedTransition"] == 0
        and protocol["clockContract"]["rawObservationLabel"]["scientificTime"] is False,
        "prohibitedCacheBoundaryExact": sorted(
            protocol["accessBoundary"]["prohibitedRoots"]
        )
        == ["/cache/e07-s12u", "/cache/e07-s12x"],
        "preregistrationPreSource": preregistration[
            "frozenBeforeProspectiveSourceModification"
        ]
        is True,
        "freezeRevalidationPassed": freeze["passed"] is True
        and freeze["prohibitedRootMetadataRead"] is False,
        "clockStatesExact": clock_states["horizonTransitions"] == 32
        and clock_states["validElapsedTransitions"] == list(range(33))
        and clock_states["rawLabelProvenance"]["controlsScientificTime"] is False,
        "s12rCommitmentsExact": commitments["candidatePopulation"]
        == {
            "lineages": 7,
            "configurations": 14,
            "adaptations": 30,
            "replacementComparators": 2,
        }
        and commitments["scenarioPopulation"]["logicalReservations"] == 195_072
        and commitments["scenarioPopulation"]["physicalReplayCommitments"] == 390_144,
        "fixedSlotsAndFamiliesExact": families["slotCount"] == 7_986
        and families["familyCount"] == 8
        and families["allSlotsRetained"] is True,
        "scientificContractPreserved": preservation["passed"] is True
        and preservation["populationChanged"] is False
        and preservation["multiplicityChanged"] is False,
        "publisherQualified": publisher["passed"] is True,
        "consumedCacheMetadataOperations": 0,
        "consumedOutcomeRowsRead": 0,
    }
    return {
        "schemaVersion": "e07.s12aa.s12z-clock-revalidation.v1",
        "researchStepId": "S12AA",
        "files": records,
        "checks": checks,
        "allPassed": all(
...
def _publisher_preflight() -> dict[str, Any]:
    ...
    payloads = {
        class_id: publication_json_bytes(
            {
                "schemaVersion": "e07.s12aa.publisher-preflight.v1",
    ...
    test_root = Path("/cache/e07-s12aa-publisher-preflight")
    if test_root.exists():
...
def _freshness_validation(raw_control: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if len(remote_lines) != 1:
        raise RuntimeError("cannot resolve the configured remote S12AA branch")
    ...
    return {
        "schemaVersion": "e07.s12aa.freshness-validation.v1",
        "researchStepId": "S12AA",
        "cachePath": str(CACHE),
...
def _s12t_dependency_preflight(
    ...
    static = StaticImportAnalyzer(
        ...
        [
            ...
            REPOSITORY / "src/morph2d/baseline.py",
            REPOSITORY / "src/morph2d/elapsed_clock.py",
        ]
    )
    session_commitment = domain_hash(
        "E07/S12AA/preflight-dependency-session/v1",
        {
    ...
    for path in (
        ...
        ledger.authorize_path(
            path,
            operation="read",
            source_api="S12AA.preflight",
        )
    ledger.authorize_signal("S12AA.human_execution_authorization")
    ...
    result = {
        "schemaVersion": "e07.s12aa.s12t-dependency-preflight.v1",
        "researchStepId": "S12AA",
...
def run_preflight() -> dict[str, Any]:
    ...
    if not freshness["passed"]:
        raise RuntimeError(f"S12AA freshness validation failed: {freshness}")
    ...
    s12w_validation = _s12w_estimator_validation()
    s12z_validation = _s12z_clock_validation()
    ...
    fixed_slot_registry = build_fixed_slot_registry(
        structural["roster"],
        structural["lineages"],
    )
    ...
    contracts = {
        ...
        "faultCostFieldsExact": tuple(
            cost_registry["faultLedger"]["separateFaultLedger"]
        )
        == FAULT_COST_FIELDS,
        "schedulerCostFieldsExact": tuple(
            cost_registry["schedulerLedger"]["separateSchedulerLedger"]
        )
        == SCHEDULER_COST_FIELDS,
    }
    contracts["passed"] = all(
        ...
        in {
            ...
            "faultCostFieldsExact",
            "schedulerCostFieldsExact",
        }
    )
    checks = {
        ...
        and s12w_validation["allPassed"]
        and s12z_validation["allPassed"],
    ...
    all_passed = bool(
        ...
        and fixed_slot_registry["slotCount"] == 7_986
        and fixed_slot_registry["familySlotCounts"]
        == read_json(S12Z_FIXED_FAMILY_FILE)["familyCounts"]
    )
    preflight = {
        "schemaVersion": "e07.s12aa.zero-episode-preflight.v1",
        "researchStepId": "S12AA",
        ...
        "s12wEstimatorQualification": s12w_validation,
        "s12zAuthenticatedElapsedClockQualification": s12z_validation,
        "frozenGateRows": gate_rows,
        "liveGateRows": checks,
        "structuralCommitments": structural["summary"],
        "fixedEstimatorSlotRegistry": {
            key: value for key, value in fixed_slot_registry.items() if key != "slots"
        },
        ...
        "substantiveTransferAuthorizedByHumanForS12AAOnly": True,
        "s12rGateFileSubstantiveAuthorizationMutated": False,
    }
    if _OUTPUT_LEDGER is None:
        raise RuntimeError("S12AA output ledger was not installed")
    _OUTPUT_LEDGER.authorize_path(
        OUT,
        operation="mkdir",
    ...
    atomic_write_json(OUT / "s12aa_execution_control.json", control)
    atomic_write_json(OUT / "preflight_validation.json", preflight)
    atomic_write_json(OUT / "fixed_estimator_slot_registry.json", fixed_slot_registry)
    atomic_write_json(
        OUT / "namespace_freshness_proof.json",
        {
            "schemaVersion": "e07.s12aa.namespace-freshness-proof.v1",
            "researchStepId": "S12AA",
            "provedBeforeArtifactRootCreation": True,
            "provedBeforeCacheRootCreation": True,
            "episodesSubmittedAtProof": 0,
            "cachePath": freshness["cachePath"],
            "artifactPath": freshness["artifactPath"],
            "cachePathAbsent": freshness["cachePathAbsent"],
            "artifactPathAbsent": freshness["artifactPathAbsent"],
            "scientificPathAbsent": freshness["scientificPathAbsent"],
            "priorGitHistoryUseAbsent": freshness["priorGitHistoryUseAbsent"],
            "priorRepositoryMentionCount": freshness["priorRepositoryMentionCount"],
            "preRegistrationRepositoryHead": freshness["preRegistrationRepositoryHead"],
            "preRegistrationTreeMatches": freshness["preRegistrationTreeMatches"],
            "repositorySourcePublishedBeforeExecution": freshness[
                "repositorySourcePublishedBeforeExecution"
            ],
            "S12UCacheInspected": False,
            "S12UOutcomeRowsOpened": 0,
            "S12XCacheInspected": False,
            "S12XOutcomeRowsOpened": 0,
            "passed": freshness["passed"],
        },
    )
    input_hash_freeze = {
        "schemaVersion": "e07.s12aa.input-hash-freeze.v1",
        "researchStepId": "S12AA",
        ...
        "files": {
            ...
            "s12wQualifiedEstimator": file_record(S12W_IMPLEMENTATION),
            "s12aaEstimatorIntegration": file_record(S12AA_ESTIMATOR),
            "s12zElapsedClockImplementation": file_record(ELAPSED_CLOCK_IMPLEMENTATION),
            "s12zProtocol": file_record(S12Z_PROTOCOL_FILE),
            "s12zPreregistrationFreeze": file_record(S12Z_PREREGISTRATION_FILE),
            "s12zPreservedS12RCommitments": file_record(S12Z_COMMITMENTS_FILE),
            "s12zValidationSummary": file_record(S12Z_VALIDATION_FILE),
            "spatialExecutionWithAuthenticatedClock": file_record(
                REPOSITORY / "src/spatial_transfer/execution.py"
            ),
    ...
    input_hash_freeze["semanticSha256"] = canonical_sha256(
        "E07/S12AA/input-hash-freeze/v1",
        input_hash_freeze,
    )
    atomic_write_json(OUT / "input_hash_freeze.json", input_hash_freeze)
    preregistration_freeze = {
        "schemaVersion": "e07.s12aa.execution-preregistration-freeze.v1",
        "researchStepId": "S12AA",
        "frozenBeforeAnyEpisode": True,
        "episodesSubmittedAtFreeze": 0,
        ...
        "executionControlFile": file_record(OUT / "s12aa_execution_control.json"),
        "preflightFile": file_record(OUT / "preflight_validation.json"),
        "namespaceFreshnessProofFile": file_record(
            OUT / "namespace_freshness_proof.json"
        ),
        "fixedEstimatorSlotRegistryFile": file_record(
            OUT / "fixed_estimator_slot_registry.json"
        ),
        "fixedEstimatorSlotCount": fixed_slot_registry["slotCount"],
        "fixedEstimatorSlotRegistrySha256": fixed_slot_registry["semanticSha256"],
    ...
    preregistration_freeze["semanticSha256"] = canonical_sha256(
        "E07/S12AA/execution-preregistration-freeze/v1",
        preregistration_freeze,
    )
    atomic_write_json(
    ...
    if not all_passed:
        atomic_write_json(
            ...
            {
                "schemaVersion": "e07.s12aa.attempt-forensics.v1",
                "status": "failed_closed_before_episode",
                "scientificPublicationRows": 0,
                "preflight": preflight,
            },
        )
        raise RuntimeError("S12AA preflight failed closed before episode submission")
    _OUTPUT_LEDGER.authorize_path(
        CACHE,
        operation="mkdir",
    ...
    return {
        ...
        "fixedSlotRegistry": fixed_slot_registry,
    }
...
def _worker(item: Mapping[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        clock_summary = json.loads(
            str(logical_result["authenticatedElapsedClockSummaryJson"])
        )
        projection_raw = logical_result["authenticatedFirstCompletionProjectionJson"]
        completion_projection = (
            None if projection_raw is None else json.loads(str(projection_raw))
        )
        if (
            logical_result["schemaVersion"] != "e07.s12z.physical-result-projection.v2"
            or clock_summary["recordCount"] != 33
            or clock_summary["horizonTransitions"] != 32
            or logical_result["rawObservationLabelsUsedAsScientificTime"] is not False
            or (
                bool(logical_result["endpointAvailable"])
                and completion_projection is None
            )
            or (
                not bool(logical_result["endpointAvailable"])
                and completion_projection is not None
            )
        ):
            raise RuntimeError("S12Z authenticated-clock result projection failed")
        logical_result.update(
            {
                "authenticatedClockRecordCount": int(clock_summary["recordCount"]),
                "authenticatedClockSummaryCommitmentSha256": str(
                    clock_summary["summaryCommitmentSha256"]
                ),
                "authenticatedClockFinalRecordCommitmentSha256": str(
                    clock_summary["finalRecordCommitmentSha256"]
                ),
                "authenticatedFirstCompletionProjectionCommitmentSha256": (
                    None
                    if completion_projection is None
                    else str(completion_projection["projectionCommitmentSha256"])
                ),
            }
        )
        ...
        if not _RUNTIME["dispatchEvidenceEmitted"]:
            dependency_evidence = {
                "schemaVersion": "e07.s12aa.worker-dispatch-dependency-evidence.v1",
        ...
        return {
            ...
            "dependencyAuditCommitment": domain_hash(
                "E07/S12AA/worker-dependency-audit/v1",
                {
                    "registrySha256": _RUNTIME["dependencyRegistry"].registry_sha256,
...
def execute_phase(
    ...
    with context.Pool(
        ...
        for batch_start in range(0, len(items), batch_size):
            ...
            if failures:
                ...
                pool.terminate()
                failure = {
                    "schemaVersion": "e07.s12aa.execution-integrity-failure.v1",
                    "phase": phase,
            ...
            if completed % 1024 < batch_size:
                ...
                print(
                    f"S12AA {phase}: {completed:,}/{len(items):,} logical; "
    ...
    dependency_summary = {
        "schemaVersion": "e07.s12aa.phase-dispatch-dependency-summary.v1",
        "phase": phase,
    ...
    dependency_summary["semanticSha256"] = domain_hash(
        "E07/S12AA/phase-dispatch-dependency-summary/v1",
        {
            "phase": phase,
            "records": [
                {
    ...
    summary = {
        "schemaVersion": "e07.s12aa.phase-execution-summary.v1",
        "phase": phase,
        "plannedLogicalRows": len(items),
        ...
        "observedCompletionOrderSha256": canonical_sha256(
            "E07/S12AA/observed-completion-order/v1",
            observed_completion_order,
        ),
        "canonicalResultSetSha256": canonical_sha256(
            "E07/S12AA/canonical-result-set/v1",
            sorted(
                zip(
...
def build_paired_estimands(
    ...
    if harm:
        ...
        classification = "null"
    return {
        "schemaVersion": "e07.s12aa.paired-estimands.v1",
        "researchStepId": "S12AA",
        "taskLocalOnly": True,
        "universalScore": None,
...
def _transfer_rows(frame: pd.DataFrame) -> list[dict[str, Any]]:
    ...
    fields = (
        ...
        "authenticatedClockRecordCount",
        "authenticatedClockSummaryCommitmentSha256",
        "authenticatedClockFinalRecordCommitmentSha256",
        "authenticatedFirstCompletionProjectionCommitmentSha256",
        "rawObservationLabelsUsedAsScientificTime",
...
def build_scientific_payloads(
    ...
    lineages: Sequence[Mapping[str, Any]],
    fixed_slot_registry: Mapping[str, Any],
    preflight: Mapping[str, Any],
    phase_summaries: Sequence[Mapping[str, Any]],
) -> tuple[dict[str, bytes], dict[str, Any]]:
    estimands = build_s12w_paired_estimands(
        frame,
        lineages,
        fixed_slot_registry,
    )
    ...
    accounting = {
        "schemaVersion": "e07.s12aa.complete-accounting.v1",
        "researchStepId": "S12AA",
        ...
        "authenticatedClockRows": int(
            frame["authenticatedElapsedClockSummaryJson"].notna().sum()
        ),
        "authenticatedClockRecordCountRows": int(
            frame["authenticatedClockRecordCount"].eq(33).sum()
        ),
        "rawObservationLabelScientificTimeRows": int(
            frame["rawObservationLabelsUsedAsScientificTime"].ne(False).sum()
        ),
    ...
    accounting["allPassed"] = bool(
        ...
        and accounting["authenticatedClockRows"] == 195_072
        and accounting["authenticatedClockRecordCountRows"] == 195_072
        and accounting["rawObservationLabelScientificTimeRows"] == 0
    )
    objects = {
        "transfer_results": {
            "schemaVersion": "e07.s12aa.transfer-results.v1",
            "researchStepId": "S12AA",
        ...
        "failure_censor_ledger": {
            "schemaVersion": "e07.s12aa.failure-censor-ledger.v1",
            "researchStepId": "S12AA",
        ...
        "native_cost_ledger": {
            "schemaVersion": "e07.s12aa.native-cost-ledger.v1",
            "researchStepId": "S12AA",
            "scalarOrUniversalCost": None,
            **native_costs,
        },
        "fault_cost_ledger": {
            "schemaVersion": "e07.s12aa.fault-cost-ledger.v1",
            "researchStepId": "S12AA",
            "scalarOrUniversalCost": None,
            **fault_costs,
        },
        "scheduler_cost_ledger": {
            "schemaVersion": "e07.s12aa.scheduler-cost-ledger.v1",
            "researchStepId": "S12AA",
            "scalarOrUniversalCost": None,
            **scheduler_costs,
        },
        "replay_audit": {
            "schemaVersion": "e07.s12aa.replay-audit.v1",
            "researchStepId": "S12AA",
        ...
        "access_and_provenance": {
            "schemaVersion": "e07.s12aa.access-provenance.v1",
            "researchStepId": "S12AA",
...
def _validate_complete_execution(
    ...
    checks = {
        ...
        "s12zPhysicalProjectionSchema": bool(
            frame["schemaVersion"].eq("e07.s12z.physical-result-projection.v2").all()
        ),
        "authenticatedClockEvidenceRetained": bool(
            frame["authenticatedElapsedClockSummaryJson"].notna().all()
            and frame["authenticatedClockRecordCount"].eq(33).all()
            and frame["authenticatedClockSummaryCommitmentSha256"].notna().all()
            and frame["authenticatedClockFinalRecordCommitmentSha256"].notna().all()
            and frame.loc[
        ...
        "rawObservationLabelsNeverScientificTime": bool(
            frame["rawObservationLabelsUsedAsScientificTime"].eq(False).all()
        ),
    ...
    return {
        "schemaVersion": "e07.s12aa.complete-execution-validation.v1",
        "researchStepId": "S12AA",
        "checks": checks,
        "allPassed": all(checks.values()),
        "logicalResultSetSha256": canonical_sha256(
            "E07/S12AA/ordered-logical-results/v1",
            sorted(
                zip(
                    frame["logicalReservationId"],
                    frame["resultProjectionSha256"],
        ...
        "physicalDispositionSetSha256": canonical_sha256(
            "E07/S12AA/ordered-physical-dispositions/v1",
            sorted(
                zip(
                    physical["physicalExecutionId"],
...
def _publish(
    ...
    _OUTPUT_LEDGER.authorize_path(
        FORENSICS,
        operation="write_atomic_publication_forensics",
        artifact_class="publication_forensics",
    )
    for spec in specs:
        _OUTPUT_LEDGER.authorize_path(
    ...
    return {
        "schemaVersion": "e07.s12aa.publication-validation.v1",
        "researchStepId": "S12AA",
        "artifactClassCount": len(specs),
        "singleCommitBoundary": audit["commitBoundaryCount"] == 1,
...
def _report_text(
    ...
    return f"""# S12AA — Fresh execution of the frozen S12R replacement spatial-transfer study

## Concise top summary

| Field | Result |
| --- | --- |
| Research step ID | **S12AA** |
...
| Validation result | **PASS — {accounting["executedLogicalReservations"]:,}/{accounting["plannedLogicalReservations"]:,} logical and {accounting["executedPhysicalReplays"]:,}/{accounting["plannedPhysicalReplays"]:,} physical commitments; exact replay, identity, S12T actual-use controls, the S12W finite-record path, the S12Z authenticated elapsed clock, protected denial, and complete-set publication all passed** |
...
- Fresh namespace: `/cache/e07-s12aa`; no predecessor cache or outcome row was
  deserialized or reused.
- Execution source: `scripts/execute_replacement_spatial_transfer_s12aa.py`,
  `src/spatial_transfer/execution.py`, `src/morph2d/elapsed_clock.py`,
  `src/morph2d/engine.py`, `src/morph2d/baseline.py`, and
  `src/morph2d/dsl_adapters.py`; the S12T actual-use dependency control, S12W
  endpoint-domain/exact-pair finite-record path, and S12Z native/DSL elapsed
  clock projection were revalidated on the real dispatch path.
- Protected validation, confirmation, the S14 reserve, civic work, S13, and
  S14 remained sealed.

## Detailed methods

Before any episode, S12AA rehashed every S12R artifact and preregistration
...
protected denial plus the complete-set publisher. It also revalidated the
S12T actual-use dependency controls, the complete S12W endpoint-domain,
exact-pairing, all-reserved, fixed-slot finite-record path, and the S12Z
authenticated clock/projection contracts. G01–G10 all passed.
...
```bash
python -m py_compile scripts/execute_replacement_spatial_transfer_s12aa.py src/spatial_transfer/execution.py
pytest -q tests/test_s12aa_replacement_execution.py tests/test_s12r_replacement_spatial_transfer.py tests/test_native_environment_suite.py
python scripts/execute_replacement_spatial_transfer_s12aa.py
```
...
| Authenticated S12Z clock rows / records per row | {accounting["authenticatedClockRows"]:,} / 33 |
...
- Native legality and the S12Z authenticated 0..32 elapsed clock, terminal
  record, and first-completion projection passed for every native/DSL run;
  raw engine-return labels were never used as time.
- The S12W endpoint-domain, exact-pair, all-reserved, fixed-slot and
  finite-record checks covered all 7,986 unchanged slots in all eight
  unchanged Holm families.
...
S12AA evaluates only the distinct S12R replacement study. It cannot repair or
...
- `s12aa_execution_control.json`: prospective authorization and implementation
  lock.
...
def _write_artifact_manifest() -> None:
    ...
    manifest = {
        "schemaVersion": "e07.s12aa.artifact-manifest.v1",
        "researchStepId": "S12AA",
...
def _failure_accounting() -> dict[str, Any]:
    phase_plans = {
        "development": 23_040,
        "post_lock_transfer": 172_032,
    }
    phases: dict[str, Any] = {}
    executed_logical = 0
    executed_physical = 0
    for phase, planned in phase_plans.items():
        summary_path = CACHE / f"{phase}_summary.json"
        failure_path = CACHE / f"{phase}_failure.json"
        progress_path = CACHE / f"{phase}_progress.json"
        if summary_path.exists():
            source = read_json(summary_path)
            logical = int(source["executedLogicalRows"])
            physical = int(source["executedPhysicalRows"])
            disposition = "phase_complete_validated"
            source_path = summary_path
        elif failure_path.exists():
            source = read_json(failure_path)
            logical = int(source["completedLogicalRows"])
            physical = int(source["completedPhysicalRows"])
            disposition = "phase_failed_closed"
            source_path = failure_path
        elif progress_path.exists():
            source = read_json(progress_path)
            logical = int(source["completedLogicalRows"])
            physical = int(source["completedPhysicalRows"])
            disposition = "last_authenticated_progress_checkpoint"
            ...
            source_path = None
        executed_logical += logical
        executed_physical += physical
        phases[phase] = {
            "plannedLogicalRows": planned,
            "plannedPhysicalRows": planned * 2,
            "accountedLogicalRows": logical,
            "accountedPhysicalRows": physical,
            "disposition": disposition,
            "source": None if source_path is None else file_record(source_path),
        }
    published_classes = 0
    if SCIENTIFIC.exists():
        published_classes = sum(
            (SCIENTIFIC / f"{class_id}.json").exists()
            for class_id in PUBLICATION_CLASSES
        )
    return {
        "schemaVersion": "e07.s12aa.fail-closed-accounting.v1",
        "researchStepId": "S12AA",
        "plannedLogicalReservations": 195_072,
        "accountedLogicalReservations": executed_logical,
        "plannedPhysicalReplays": 390_144,
        "accountedPhysicalReplays": executed_physical,
        "phases": phases,
        "scientificArtifactClassesPublished": published_classes,
        "completeScientificPublication": published_classes == 10,
        "validationOutcomeRowsRead": 0,
        "confirmationOutcomeRowsRead": 0,
        "protectedOutcomeRowsRead": 0,
        "S12UCacheReads": 0,
...
def fail_closed(
    ...
    *,
    ...
    accounting = _failure_accounting()
    atomic_write_json(OUT / "execution_accounting.json", accounting)
    if _OUTPUT_LEDGER is not None:
        try:
    ...
    report = f"""# S12AA — Fresh execution of the frozen S12R replacement spatial-transfer study
...
S12AA used only the frozen S12R/S12T/S12W/S12Z controls and the prospectively
registered `/cache/e07-s12aa` namespace. It performed zero metadata operations
on, and read zero outcome rows from, the consumed S12U and S12X caches. Source
and control hashes were frozen in `s12aa_execution_control.json`; predecessor
immutability and protected-denial evidence are retained in the reached
preflight or forensic artifacts.

## Detailed methods

Before an episode, the executor required a fresh artifact/cache proof, exact
G01–G10 validation, all 3,120 bindings, seven lineages, 14 configurations, 30
adaptations, two replacement comparators, 48 task-condition cells, 3,840
scenario families, 195,072 logical reservations, 390,144 physical
commitments, the S12T actual-use controls, the S12W all-reserved finite-record
path, the S12Z authenticated clock/projection, all 7,986 fixed slots in eight
Holm families, and the ten-class complete-or-zero publisher. The first failed
integrity assertion invoked this terminal fail-closed path; no repair, resume,
...
The failing condition was `{type(error).__name__}: {error}`. The scientific
publication state is `{scientific_state}`. Exact authenticated accounting
reached {accounting["accountedLogicalReservations"]:,}/195,072 logical and
{accounting["accountedPhysicalReplays"]:,}/390,144 physical commitments.
Inspect `attempt_forensics.json` and `execution_accounting.json` for the exact
phase disposition.

## Validation result

Validation failed at the first integrity defect. The attempt is quarantined,
and the atomic boundary exposed zero scientific classes unless a complete
validated ten-class set had already committed before the later handoff fault.
No incomplete scientific class is valid.

## Caveats and blockers

This failed attempt supplies forensic evidence only and cannot support an
efficacy or transfer claim. Protected validation, confirmation, S14 reserve,
civic, S13, and S14 outcome counts remain zero. Human review is required; no
automatic downstream step or additional attempt is authorized.

## Artifact provenance

`attempt_forensics.json` records the exception, elapsed duration, publication
state, fresh namespace, and sealed-domain counts. `execution_accounting.json`
records phase-local planned and authenticated completed commitments. The
artifact manifest hashes every compact handoff file that was successfully
written.
"""
    ...
    status = _status(
        ...
        classification="constraining/contradictory",
        caveats=["Fresh S12AA attempt quarantined; no partial result is valid."],
...
def run() -> None:
    ...
    try:
        ...
        payloads, summary = build_scientific_payloads(
            ...
            preflight_data["fixedSlotRegistry"],
        ...
        if (
            summary["estimands"]["recordCount"] != 7_986
            or len(summary["estimands"]["fixedFamilies"]) != 8
            or set(summary["estimands"]["fixedFamilies"])
            != set(preflight_data["fixedSlotRegistry"]["fixedFamilies"])
            or len(payloads) != 10
            or set(payloads) != set(PUBLICATION_CLASSES)
        ):
            raise RuntimeError(
                "fixed-slot, Holm-family, or ten-class publication conservation failed"
            )
        ...
        atomic_write_json(
            ...
            {
                "schemaVersion": "e07.s12aa.outcome-summary.v1",
                "researchStepId": "S12AA",
                **summary["estimands"],
        ...
        provenance = {
            "schemaVersion": "e07.s12aa.provenance.v1",
            "researchStepId": "S12AA",
            "timestampUtc": datetime.now(timezone.utc).isoformat(),
        ...
        status = _status(
            ...
            caveats=[
                ...
                (
                    "Authenticated S12AA output ledger validation: "
...
def validate_existing() -> None:
    required = (
        OUT / "s12aa_execution_control.json",
    ...
    if missing:
        raise RuntimeError(f"S12AA validation missing files: {missing}")
    ...
    if not (
        ...
        raise RuntimeError("S12AA final validation failed")
    print("S12AA validation PASS", flush=True)
