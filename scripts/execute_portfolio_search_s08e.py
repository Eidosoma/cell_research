#!/usr/bin/env python3
"""Execute fresh S08E under the byte-frozen S08P design and S08D gate."""
...
S08E_CONTROL_PATH = (
    ...
    / "configs/portfolio/s08e_execution_continuation.yaml"
)
S08E_CACHE_ROOT = Path("/cache/e07-s08e")
IMMUTABLE_STEPS = ("S05", "S08P", "S08A", "S08", "S08B", "S08C", "S08D")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser()
    parser.add_argument("--output", type=Path, default=ARTIFACT_ROOT / "S08E")
    parser.add_argument(
...
def work_digest(rows: Sequence[Mapping[str, Any]]) -> str:
    return canonical_hash(
        "E07/S08E/work-order-independent/v1",
        sorted(
            (
...
def result_digest(rows: Sequence[Mapping[str, Any]]) -> str:
    return canonical_hash(
        "E07/S08E/result-order-independent/v1",
        sorted(
...
def configuration_hash_validation(
        ...
        rows.append(
            {
                ...
                "configurationDefinitionSha256": canonical_hash(
                    "E07/S08E/configuration-definition/v1", definition
                ),
    ...
    return {
        "schemaVersion": "e07.s08e.configuration-hash-validation.v1",
        "researchStepId": "S08E",
        "success": all(
...
def validation_support_audit(rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    for task_id, task_rows in sorted(by_task.items()):
        ...
        support = {
            canonical_hash(
                "E07/S08E/native-validation-support/v1",
                {
                    "outcome": row["outcome"],
    ...
    return {
        "schemaVersion": "e07.s08e.validation-support-audit.v1",
        "researchStepId": "S08E",
...
def main() -> int:
    ...
    for key in ("OMP_NUM_THREADS", "MKL_NUM_THREADS", "OPENBLAS_NUM_THREADS"):
        os.environ[key] = "1"
    output = args.output
    if args.phase != "preflight" and S08E_CACHE_ROOT.exists():
        raise SystemExit(
            f"fresh S08E cache namespace already exists: {S08E_CACHE_ROOT}"
        )
    output.mkdir(parents=True, exist_ok=True)
    control = yaml.safe_load(S08E_CONTROL_PATH.read_text(encoding="utf-8"))
    immutable_expected = dict(control["expectedTrees"])
    preflight = run_preflight(S08E_CONTROL_PATH)
    _write_json(output / "preflight_hash_and_gate_revalidation.json", preflight)
    _write_json(
        output / "s08d_binding_revalidation.json", preflight["bindingValidation"]
    )
    _write_json(
        output / "access_control_preflight.json",
        {
            "schemaVersion": "e07.s08e.access-control-preflight.v1",
            "researchStepId": "S08E",
            "success": all(
    ...
    try:
        smoke_rows, smoke_physical = execute_work(
            ...
            cache_dir=S08E_CACHE_ROOT / "evaluations",
    ...
    smoke_validation.update(
        {
            "schemaVersion": "e07.s08e.frozen-smoke-validation.v1",
            "researchStepId": "S08E",
    ...
    if not smoke_validation["success"]:
        _write_json(
            ...
            {
                "schemaVersion": "e07.s08e.smoke-forensic-accounting.v1",
                "researchStepId": "S08E",
                "success": False,
    ...
    training_rows, initial_physical = execute_work(
        initial_work, workers=args.workers, cache_dir=S08E_CACHE_ROOT / "evaluations"
    )
    ...
    for generation in range(1, 7):
        ...
        rows, physical = execute_work(
            works, workers=args.workers, cache_dir=S08E_CACHE_ROOT / "evaluations"
        )
        ...
        checkpoint = S08E_CACHE_ROOT / f"generation_{generation}_checkpoint.json"
        _write_json(
            checkpoint,
            {
                "generation": generation,
    ...
    training_validation.update(
        {
            "schemaVersion": "e07.s08e.training-validation.v1",
            "researchStepId": "S08E",
    ...
    validation_rows, validation_physical = execute_work(
        validation_work,
        workers=args.workers,
        cache_dir=S08E_CACHE_ROOT / "validation_evaluations",
    )
    ...
    gate["schemaVersion"] = "e07.s08e.s09-eligibility-gate.v1"
    gate["researchStepId"] = "S08E"
    ...
    _write_json(
        ...
        {
            "schemaVersion": "e07.s08e.convergence.v1",
            "researchStepId": "S08E",
            "rows": convergence,
    ...
    _write_json(
        ...
        {
            "schemaVersion": "e07.s08e.training-shortlist.v1",
            "researchStepId": "S08E",
            "byTask": shortlist,
    ...
    _write_json(
        ...
        {
            "schemaVersion": "e07.s08e.validation-plan-accounting.v1",
            "researchStepId": "S08E",
    ...
    _write_json(
        ...
        {
            "schemaVersion": "e07.s08e.uncertainty-multiplicity.v1",
            "researchStepId": "S08E",
    ...
    for aggregate in aggregates:
        signature = canonical_hash(
            "E07/S08E/finite-panel-equivalence/v1",
    ...
    _write_json(
        ...
        {
            "schemaVersion": "e07.s08e.duplicate-audit.v1",
            "researchStepId": "S08E",
    ...
    order_validation = {
        "schemaVersion": "e07.s08e.replay-worker-order-validation.v1",
        "researchStepId": "S08E",
    ...
    budget_accounting = {
        "schemaVersion": "e07.s08e.complete-budget-accounting.v1",
        "researchStepId": "S08E",
        "trainingLogicalRows": len(training_rows),
    ...
    _write_json(
        ...
        {
            "schemaVersion": "e07.s08e.access-control-validation.v1",
            "researchStepId": "S08E",
    ...
    _write_json(
        ...
        {
            "schemaVersion": "e07.s08e.dependency-exclusion-audit.v1",
            "researchStepId": "S08E",
    ...
    _write_json(
        ...
        {
            "schemaVersion": "e07.s08e.no-mutation-audit.v1",
            "researchStepId": "S08E",
    ...
    _write_json(
        ...
        {
            "schemaVersion": "e07.s08e.provenance.v1",
            "researchStepId": "S08E",
            "controlPath": str(S08E_CONTROL_PATH),
            "controlSha256": sha256_file(S08E_CONTROL_PATH),
    ...
    summary = {
        "schemaVersion": "e07.s08e.validation-summary.v1",
        "researchStepId": "S08E",
        "checks": {
