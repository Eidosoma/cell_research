#!/usr/bin/env python3
"""Execute one frozen S11 confirmatory increment with restartable checkpoints."""
...
SETTING_FIELDS = (
    ...
    "informationPermission", "legalPrimitives", "proposalCandidatesPerOpportunity",
)


def write_parquet(path: Path, rows: list[Mapping[str, Any]]) -> None:
    pq.write_table(
        pa.Table.from_pylist(list(rows)), path,
...
def execute(payload: tuple[dict[str, Any], dict[str, Any], dict[str, Any]]) -> dict[str, Any]:
    ...
    row = flat_screening_row(run_design, result)
    row["schemaVersion"] = "e02.s11.confirmatory_run.v1"
    return row
...
def run_stage(args: argparse.Namespace) -> None:
    ...
    design = pq.read_table(
        args.design, filters=[("confirmatoryLook", "=", args.stage)]
    ).to_pylist()
    pairing_rows = pq.read_table(
        args.pairing, filters=[("confirmatoryLook", "=", args.stage)]
    ).to_pylist()
    ...
    scenario_table = pq.read_table(
        args.scenario_bank, filters=[("split", "=", "confirmatory_holdout")]
    )
    mask = pc.is_in(
        scenario_table["inputScenarioId"], value_set=pa.array(sorted(input_ids))
    )
    scenarios = {
        str(row["inputScenarioId"]): row
        for row in scenario_table.filter(mask).to_pylist()
    }
    if len(design) != 6000 or len(pairing) != 1000 or len(scenarios) != 1000:
        raise AssertionError(
            f"stage accounting mismatch: design={len(design)}, pairing={len(pairing)}, scenarios={len(scenarios)}"
        )
    if not all(row["protected"] and row["split"] == "confirmatory_holdout" for row in design):
        raise AssertionError("S11 stage contains a nonprotected run")
    ...
    if existing_rows:
        print(json.dumps({
            "stage": args.stage, "status": "resume_existing",
            "checkpoints": len(stage_paths), "rows": len(existing_rows),
    ...
    payloads = [
        ...
        for row in design if row["runDesignId"] not in existing_set
    ]
    next_checkpoint = max(
        (int(path.stem.rsplit("_", 1)[1]) for path in stage_paths), default=-1
    ) + 1
    ...
    def checkpoint(*, final: bool = False) -> None:
        ...
        while len(buffer) >= args.chunk_size or (final and buffer):
            ...
            if not all(row["contractValidationPass"] for row in rows):
                raise AssertionError("confirmatory checkpoint failed contract validation")
            ...
            print(json.dumps({
                "stage": args.stage, "chunk": next_checkpoint,
                "status": "written", "rows": len(rows),
                "scientificRunTimeSeconds": sum(float(row["wallTimeSeconds"]) for row in rows),
            }, sort_keys=True), flush=True)
            next_checkpoint += 1
    ...
    stage_paths = sorted(args.cache.glob(f"stage_{args.stage:02d}_chunk_*.parquet"))
    combined = pa.concat_tables([pq.read_table(path) for path in stage_paths]).sort_by([
        ("runDesignId", "ascending")
    ])
    if combined.num_rows != 6000 or len(set(combined["runDesignId"].to_pylist())) != 6000:
        raise AssertionError("combined confirmatory-stage accounting failed")
    stage_output = args.cache / f"stage_{args.stage:02d}_confirmatory_runs.parquet"
    pq.write_table(
        combined, stage_output,
        compression="zstd", compression_level=9, use_dictionary=True,
        write_statistics=True, version="2.6",
    )
    print(json.dumps({
        "stage": args.stage, "status": "complete", "rows": combined.num_rows,
