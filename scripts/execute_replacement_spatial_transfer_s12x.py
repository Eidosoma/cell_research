"""Execute the exact byte-frozen S12R replacement study as fresh step S12X."""
...
OUT = ARTIFACTS / "research_steps/S12X"
S12R = ARTIFACTS / "research_steps/S12R"
CACHE = Path("/cache/e07-s12x")
CONTROL = REPOSITORY / "configs/transfer/s12x_fresh_s12r_execution.yaml"
OUTPUT_REGISTRY_FILE = REPOSITORY / "configs/transfer/s12x_output_access_registry.json"
...
S12X_ESTIMATOR = REPOSITORY / "src/spatial_transfer/s12x_estimands.py"
...
def _finalize_output_access_ledger(*, disposition: str) -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError("S12X output access ledger is not installed")
    ...
    if not validation["passed"]:
        raise RuntimeError("S12X authenticated output ledger failed validation")
    return validation
...
def control_record() -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError("S12X execution control is not the authorized continuation")
    body = {
        "schemaVersion": "e07.s12x.execution-control-record.v1",
        "researchStepId": "S12X",
    ...
    body["executionControlSha256"] = canonical_sha256(
        "E07/S12X/execution-control/v1", body
    )
    return body
...
def _predecessor_immutability_validation() -> dict[str, Any]:
    # Revalidate predecessor immutability roots only.  In particular, never
    # open an S12U outcome-bearing artifact or its forbidden cache.
    prior = read_json(S12W / "predecessor_immutability_validation.json")
    expected_manifest_hashes = {
        str(row["researchStepId"]): str(row["expectedSha256"])
        for row in prior["rows"]
    }
    expected_manifest_hashes["S12W"] = yaml.safe_load(
        CONTROL.read_text(encoding="utf-8")
    )["frozenS12W"]["artifactManifestSha256"]
    validations = {}
    ...
    result = {
        "schemaVersion": "e07.s12x.predecessor-immutability-validation.v1",
        "researchStepId": "S12X",
...
def _s12w_estimator_validation() -> dict[str, Any]:
    ...
    expected = {
        ...
        "qualifiedImplementation": raw["implementationSha256"],
    }
    records = {
        key: {
            **file_record(path),
    ...
    checks = {
        ...
        "fixedFamiliesExact": commitment["fixedFamilies"]
        == raw["fixedFamilies"],
        "holmExact": (
            commitment["multiplicityMethod"] == "Holm"
            and commitment["alpha"] == 0.05
        ),
    ...
    return {
        "schemaVersion": "e07.s12x.s12w-estimator-revalidation.v1",
        "researchStepId": "S12X",
        "files": records,
        "checks": checks,
...
def _publisher_preflight() -> dict[str, Any]:
    ...
    payloads = {
        class_id: publication_json_bytes(
            {
                "schemaVersion": "e07.s12x.publisher-preflight.v1",
    ...
    test_root = Path("/cache/e07-s12x-publisher-preflight")
    if test_root.exists():
...
def _freshness_validation(raw_control: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if len(remote_lines) != 1:
        raise RuntimeError("cannot resolve the configured remote S12X branch")
    ...
    return {
        "schemaVersion": "e07.s12x.freshness-validation.v1",
        "researchStepId": "S12X",
        "cachePath": str(CACHE),
...
def _s12t_dependency_preflight(
    ...
    session_commitment = domain_hash(
        "E07/S12X/preflight-dependency-session/v1",
        {
    ...
    for path in (
        ...
        ledger.authorize_path(
            path,
            operation="read",
            source_api="S12X.preflight",
        )
    ledger.authorize_signal("S12X.human_execution_authorization")
    ...
    result = {
        "schemaVersion": "e07.s12x.s12t-dependency-preflight.v1",
        "researchStepId": "S12X",
...
def run_preflight() -> dict[str, Any]:
    ...
    if not freshness["passed"]:
        raise RuntimeError(f"S12X freshness validation failed: {freshness}")
    ...
    all_passed = bool(
        ...
        and fixed_slot_registry["slotCount"] > 0
        and all(
            fixed_slot_registry["familySlotCounts"][family] > 0
            for family in fixed_slot_registry["fixedFamilies"]
        )
    )
    preflight = {
        "schemaVersion": "e07.s12x.zero-episode-preflight.v1",
        "researchStepId": "S12X",
        ...
        "fixedEstimatorSlotRegistry": {
            key: value
            for key, value in fixed_slot_registry.items()
            if key != "slots"
        },
        ...
        "substantiveTransferAuthorizedByHumanForS12XOnly": True,
        "s12rGateFileSubstantiveAuthorizationMutated": False,
    }
    if _OUTPUT_LEDGER is None:
        raise RuntimeError("S12X output ledger was not installed")
    _OUTPUT_LEDGER.authorize_path(
        OUT,
        operation="mkdir",
    ...
    atomic_write_json(OUT / "s12x_execution_control.json", control)
    ...
    atomic_write_json(
        ...
        {
            "schemaVersion": "e07.s12x.namespace-freshness-proof.v1",
            "researchStepId": "S12X",
            ...
            "priorRepositoryMentionCount": freshness[
                "priorRepositoryMentionCount"
            ],
            "preRegistrationRepositoryHead": freshness[
                "preRegistrationRepositoryHead"
            ],
            "preRegistrationTreeMatches": freshness[
                "preRegistrationTreeMatches"
            ],
    ...
    input_hash_freeze = {
        "schemaVersion": "e07.s12x.input-hash-freeze.v1",
        "researchStepId": "S12X",
        ...
        "files": {
            ...
            "s12xEstimatorIntegration": file_record(S12X_ESTIMATOR),
    ...
    input_hash_freeze["semanticSha256"] = canonical_sha256(
        "E07/S12X/input-hash-freeze/v1",
        input_hash_freeze,
    )
    atomic_write_json(OUT / "input_hash_freeze.json", input_hash_freeze)
    preregistration_freeze = {
        "schemaVersion": "e07.s12x.execution-preregistration-freeze.v1",
        "researchStepId": "S12X",
        "frozenBeforeAnyEpisode": True,
        "episodesSubmittedAtFreeze": 0,
        ...
        "executionControlFile": file_record(OUT / "s12x_execution_control.json"),
    ...
    preregistration_freeze["semanticSha256"] = canonical_sha256(
        "E07/S12X/execution-preregistration-freeze/v1",
        preregistration_freeze,
    )
    atomic_write_json(
    ...
    if not all_passed:
        atomic_write_json(
            ...
            {
                "schemaVersion": "e07.s12x.attempt-forensics.v1",
                "status": "failed_closed_before_episode",
                "scientificPublicationRows": 0,
                "preflight": preflight,
            },
        )
        raise RuntimeError("S12X preflight failed closed before episode submission")
    _OUTPUT_LEDGER.authorize_path(
        CACHE,
        operation="mkdir",
...
def _worker(item: Mapping[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        if not _RUNTIME["dispatchEvidenceEmitted"]:
            dependency_evidence = {
                "schemaVersion": "e07.s12x.worker-dispatch-dependency-evidence.v1",
        ...
        return {
            ...
            "dependencyAuditCommitment": domain_hash(
                "E07/S12X/worker-dependency-audit/v1",
                {
                    "registrySha256": _RUNTIME["dependencyRegistry"].registry_sha256,
...
def execute_phase(
    ...
    with context.Pool(
        ...
        for batch_start in range(0, len(items), batch_size):
            ...
            if failures:
                ...
                pool.terminate()
                failure = {
                    "schemaVersion": "e07.s12x.execution-integrity-failure.v1",
                    "phase": phase,
            ...
            if completed % 1024 < batch_size:
                ...
                print(
                    f"S12X {phase}: {completed:,}/{len(items):,} logical; "
    ...
    dependency_summary = {
        "schemaVersion": "e07.s12x.phase-dispatch-dependency-summary.v1",
        "phase": phase,
    ...
    dependency_summary["semanticSha256"] = domain_hash(
        "E07/S12X/phase-dispatch-dependency-summary/v1",
        {
            "phase": phase,
            "records": [
                {
    ...
    summary = {
        "schemaVersion": "e07.s12x.phase-execution-summary.v1",
        "phase": phase,
        "plannedLogicalRows": len(items),
        ...
        "observedCompletionOrderSha256": canonical_sha256(
            "E07/S12X/observed-completion-order/v1",
            observed_completion_order,
        ),
        "canonicalResultSetSha256": canonical_sha256(
            "E07/S12X/canonical-result-set/v1",
            sorted(
                zip(
...
def build_paired_estimands(
    ...
    if harm:
        ...
        classification = "null"
    return {
        "schemaVersion": "e07.s12x.paired-estimands.v1",
        "researchStepId": "S12X",
        "taskLocalOnly": True,
        "universalScore": None,
...
def build_scientific_payloads(
    ...
    accounting = {
        "schemaVersion": "e07.s12x.complete-accounting.v1",
        "researchStepId": "S12X",
    ...
    objects = {
        "transfer_results": {
            "schemaVersion": "e07.s12x.transfer-results.v1",
            "researchStepId": "S12X",
        ...
        "failure_censor_ledger": {
            "schemaVersion": "e07.s12x.failure-censor-ledger.v1",
            "researchStepId": "S12X",
        ...
        "native_cost_ledger": {
            "schemaVersion": "e07.s12x.native-cost-ledger.v1",
            "researchStepId": "S12X",
            "scalarOrUniversalCost": None,
            **native_costs,
        },
        "fault_cost_ledger": {
            "schemaVersion": "e07.s12x.fault-cost-ledger.v1",
            "researchStepId": "S12X",
            "scalarOrUniversalCost": None,
            **fault_costs,
        },
        "scheduler_cost_ledger": {
            "schemaVersion": "e07.s12x.scheduler-cost-ledger.v1",
            "researchStepId": "S12X",
            "scalarOrUniversalCost": None,
            **scheduler_costs,
        },
        "replay_audit": {
            "schemaVersion": "e07.s12x.replay-audit.v1",
            "researchStepId": "S12X",
        ...
        "access_and_provenance": {
            "schemaVersion": "e07.s12x.access-provenance.v1",
            "researchStepId": "S12X",
...
def _validate_complete_execution(
    ...
    return {
        "schemaVersion": "e07.s12x.complete-execution-validation.v1",
        "researchStepId": "S12X",
        "checks": checks,
        "allPassed": all(checks.values()),
        "logicalResultSetSha256": canonical_sha256(
            "E07/S12X/ordered-logical-results/v1",
            sorted(
                zip(
                    frame["logicalReservationId"],
                    frame["resultProjectionSha256"],
        ...
        "physicalDispositionSetSha256": canonical_sha256(
            "E07/S12X/ordered-physical-dispositions/v1",
            sorted(
                zip(
                    physical["physicalExecutionId"],
...
def _publish(
    ...
    return {
        "schemaVersion": "e07.s12x.publication-validation.v1",
        "researchStepId": "S12X",
        "artifactClassCount": len(specs),
        "singleCommitBoundary": audit["commitBoundaryCount"] == 1,
...
def _report_text(
    ...
    return f"""# S12X — Fresh execution of the frozen S12R replacement spatial-transfer study

## Concise top summary

| Field | Result |
| --- | --- |
| Research step ID | **S12X** |
...
- Fresh namespace: `/cache/e07-s12x`; no predecessor cache or outcome row was
  deserialized or reused.
- Execution source: `scripts/execute_replacement_spatial_transfer_s12x.py`,
...
Before any episode, S12X rehashed every S12R artifact and preregistration
...
```bash
python -m py_compile scripts/execute_replacement_spatial_transfer_s12x.py src/spatial_transfer/execution.py
pytest -q tests/test_s12x_replacement_execution.py tests/test_s12r_replacement_spatial_transfer.py tests/test_native_environment_suite.py
python scripts/execute_replacement_spatial_transfer_s12x.py
```
...
S12X evaluates only the distinct S12R replacement study. It cannot repair or
...
- `s12x_execution_control.json`: prospective authorization and implementation
  lock.
...
def _write_artifact_manifest() -> None:
    ...
    manifest = {
        "schemaVersion": "e07.s12x.artifact-manifest.v1",
        "researchStepId": "S12X",
...
def _failure_accounting() -> dict[str, Any]:
    ...
    return {
        "schemaVersion": "e07.s12x.fail-closed-accounting.v1",
        "researchStepId": "S12X",
        "plannedLogicalReservations": 195_072,
...
def fail_closed(
    ...
    *,
    ...
    report = f"""# S12X — Fresh execution of the frozen S12R replacement spatial-transfer study
...
S12X used the prospectively registered `/cache/e07-s12x` namespace and the
byte-frozen S12R protocol. The failing condition was:
`{type(error).__name__}: {error}`. The scientific publication state is
`{scientific_state}`. Exact authenticated accounting reached
{accounting["accountedLogicalReservations"]:,}/195,072 logical and
{accounting["accountedPhysicalReplays"]:,}/390,144 physical commitments.
Protected validation, confirmation, S14 reserve, civic, S13, and S14 outcome
counts remain zero. Inspect `attempt_forensics.json` and
`execution_accounting.json` for the exact phase disposition.
"""
    ...
    status = _status(
        ...
        classification="constraining/contradictory",
        caveats=["Fresh S12X attempt quarantined; no partial result is valid."],
...
def run() -> None:
    ...
    try:
        ...
        atomic_write_json(
            ...
            {
                "schemaVersion": "e07.s12x.outcome-summary.v1",
                "researchStepId": "S12X",
                **summary["estimands"],
        ...
        provenance = {
            "schemaVersion": "e07.s12x.provenance.v1",
            "researchStepId": "S12X",
            "timestampUtc": datetime.now(timezone.utc).isoformat(),
        ...
        status = _status(
            ...
            caveats=[
                ...
                (
                    "Authenticated S12X output ledger validation: "
...
def validate_existing() -> None:
    required = (
        OUT / "s12x_execution_control.json",
    ...
    if missing:
        raise RuntimeError(f"S12X validation missing files: {missing}")
    ...
    if not (
        ...
        raise RuntimeError("S12X final validation failed")
    print("S12X validation PASS", flush=True)
